...
The Table inspector consumes this API and expands each field’s raw sources with archive/hash/path labels. Unified rows select rollout text evidence when rollout text is nonempty, otherwise item text evidence; all other fields retain item provenance. Source contents are escaped as text. The independent Raw JSONL and Normalized JSON views retain their meanings.

...
| `event_msg` / `token_count` | Emits `event`, name `Token usage`, `attributes.info=payload.info`. | Normalized events preserve reported info. The separate archive-based [usage report](#71-token-usage-and-api-value) computes totals and API value. |
...
**Existing data:** explicitly reimport the original rollout with `agentboard --config config/dev.toml import PATH`. No startup migration renormalizes events from archives. Matching input IDs/text/start timestamps receive current kind, name, attribution, and source links; conflicting text stays retained. For an imported prefix without conflicting [recorded line fingerprints](#8-identity-transactions-and-upgrades), remove superseded mirrors and recompute estimated LLM gaps/between-turn waits within that prefix, preserving later events. Surviving event IDs and row cursors stay stable; removed duplicate/derived rows do not. Internal blocking request classifications are corrected on matching reimport while preserving completed results during shorter retries. An excluded input-derived title is replaced by the first retained human prompt or `Untitled session`.
...
**Mandatory capture — Implemented 2026-09-08 (schema v9):** [core ingestion](../backend/agentboard/ingestion.py) captures exact supplied bytes before UTF-8, gzip, protobuf/JSON decoding or normalization. This applies to configured Codex imports and OTLP receivers even with all analysis/inspection off. `raw_archive` gates inspection only; downloads also require `export`.

`captured_payloads` stores source (`codex`, `otlp_logs`, `otlp_traces`), content type/encoding, SHA-256, byte count and a BLOB. Identity is the source/type/encoding/hash tuple; retries reuse bytes. `captures` separately records each attempt's time, payload ID, status (`pending`, `normalized`, `failed`), result and diagnostic. Original wire bytes include gzip envelopes and unknown protobuf/JSON fields. CLI uses a stable binary temporary-file snapshot; no decoding precedes capture. Filename and arbitrary HTTP headers are not stored.

Capture commits use WAL/`synchronous=FULL` before the canonical transaction; interpretation errors roll back canonical writes while retained bytes survive. HTTP import results and handled interpretation errors include `capture_id`, `capture_status` and `normalization_status`; OTLP responses also identify the capture in `X-AgentBoard-Capture-ID`. Normalization success means supported mapping completed, not that every record was recognized. `pending` is an unknown completion state after interruption or outcome-update failure. Canonical/outcome commits retain SQLite `NORMAL` semantics and may require a retry after power loss; no automatic retry or backfill runs.

Admission rejects unavailable sources, unsupported media/encoding, oversized wire bodies and saturation without claiming capture. If a fully captured gzip payload expands over the limit, normalization returns 413 and the original compressed payload remains retained. Capture/storage failures fail explicitly; retry only after resolving capacity. Bytes never supplied by the producer, including upstream-redacted content, cannot be recovered.

Inspect attempts with `GET /api/v1/captures?after=0&limit=50` and `/captures/{capture_id}` or `agentboard captures`. Export exact transport bytes with `/captures/{capture_id}/raw` or `agentboard capture-export ID > payload.bin`; content is octet-stream to avoid automatic decompression. `agentboard reprocess ID` explicitly reparses retained bytes using currently enabled source/current mappings and creates another attempt. Inspection need not be enabled for reprocessing. Existing event merge rules still apply; this is not a general rewritten-history correction engine. The capture API does not yet link individual normalized OTLP fields to byte offsets.

Schema v9 preserves prior archives and adds empty capture tables for older data. It cannot recreate uncaptured history or original OTLP wire bytes. [Capture regressions](../backend/tests/test_capture.py) verify exact bytes, unknown content, failure/retry outcomes and migration.
...
Schema v3 adds the archive tables without reconstructing old evidence. Reimport original files to backfill older sessions and metadata. Line archives cover successful Codex JSONL normalization; independent capture also covers failed imports and complete OTLP wire bytes. Successful Codex imports currently duplicate original bytes between payload and line storage. Lossless consolidation is future optimization work. Exact round-trip, changed/shortened retry, rollback, CLI newline preservation, and migration coverage: [synthetic tests](../backend/tests/test_raw_traces.py).

### 3.6 Per-field Codex lineage

Detailed field-source computation, persistence, and these APIs require `field_lineage`; raw-inspection APIs need not be enabled. When disabled, existing mappings remain stored, with stale mappings removed if their normalized values change; ordinary event inspection and normalization continue. Enabling the feature does not automatically reconstruct missing mappings.
...
Implemented **2026-09-07**, updated **2026-09-16** to analysis version `codex-usage-v4`. [`usage.py`](../backend/agentboard/usage.py) reads one raw archive in physical line order. The default is the highest archive ID; `import_id` pins a version. Archive identity/hash, analysis version and the full dated price catalog accompany the report. No stored events, schema or Codex normalization mapping change. Existing archives work without reimport; unarchived sessions require explicit reimport. Normalized timeline filters and OTel records do not affect this report.

| Evidence | Transformation and limits |
| --- | --- |
| `token_usage_record.payload.usage` | Preferred per-response counts. Deduplicate by nonempty `response_id`; identical retries are ignored, conflicting counts retain the first and mark coverage partial. Missing IDs/invalid counts are excluded with findings. |
| `payload.thread_token_usage`; `event_msg/token_count.payload.info.total_token_usage` | When response records exist, cumulative accounts are never added. A larger cumulative total flags tokens outside the response records; no model or cost is invented for them. Consistency checks also compare valid summaries with preceding deduplicated response counts: explicit thread/turn counters use their stated scope; token-count mirrors may match either the thread or the identified turn. Known input/output and optional breakdowns must agree; missing optional fields are not assumed zero. Mismatches mark coverage partial and show a warning, with summary line, scope, turn and counters in `usage_summary_mismatches` in API/export. These checks do not establish source-file completeness or change imported lines or response totals. |
| Legacy-only `info.total_token_usage` | Difference successive counters; initial baseline is zero. Ignore repeated counters; skip a decreasing counter, rebase and mark partial. `last_token_usage` matching the input/output delta establishes a single-request delta. Otherwise the delta's model and per-request context size remain unknown. Last-only notifications cannot safely establish additive usage. |
| `usage` fields | Nonnegative integers only. `input_tokens` includes `cached_input_tokens` and `cache_write_input_tokens`; `output_tokens` includes `reasoning_output_tokens`. Total is input + output. Inconsistent totals/subsets are rejected from analysis. Missing optional breakdowns stay null; missing required input/output invalidates a record. |
| `breakdown` at report/model/row scope | Calculated `uncached_input_tokens = input_tokens − cached_input_tokens`; this includes cache writes. `reasoning_percent_of_total = 100 × reasoning_output_tokens / total_tokens`, using input + output as the denominator. Aggregate percentages use summed counts, not a mean of row percentages. Missing operands or a zero denominator produce null; known zero reasoning with a positive total produces 0%. Raw inclusive `tokens.input_tokens` is unchanged. |
...
**Legacy context correction (`codex-usage-v4`):** retain valid `last_token_usage.input_tokens` with the notification's chronological model (or explicit override) as request-size evidence, even when the cumulative delta spans several requests and its model remains unknown. A recorded large last request establishes that model's session-wide long tier. Otherwise, a multi-request delta whose total input exceeds a model's threshold leaves its session tier unavailable: unidentified requests could belong to that model. Deltas at or below the threshold cannot contain a larger request. Per-request tiers and preferred response records keep their existing rules. These decisions precede dashboard date/model filtering.

Synthetic example: a GPT-5.4 cumulative delta of 600,000 input tokens with a 300,000-input-token last request establishes the long tier. A later 1,000-input/100-output request with no cached tokens costs `$0.00725`, rather than `$0.004`. If only a short last request is recorded, the earlier requests' sizes remain unknown and the later session-tier estimate is unavailable. Reports recalculate existing archives; no reimport or rate-card change is required.

Worked synthetic example, GPT-5.6 Sol: 1,000 input including 200 cached and 100 cache writes, plus 100 output including 40 reasoning, gives **1,100 total tokens** and `(700×4 + 200×0.4 + 100×5 + 100×20)/1,000,000 = $0.00538`. The UI splits this into **800 uncached input**, **200 cached input** and **100 output**, with **3.64% reasoning / total**. The 40 reasoning tokens are already in output. The [browser demo](../frontend/demo.jsonl) contains separately labeled synthetic usage and uses a new identity to avoid reimport conflicts with older demo line positions.

`GET /api/v1/sessions/{id}/usage` returns full-archive totals, model groups and paginated usage rows (`after` source line, `limit` up to 1,000). Pin the returned `archive.id` with `import_id` across pages. `/usage/export` exports all rows as JSON; both accept `model_override`. `/api/v1/pricing` exposes rates and their source. The UI pins the selected archive for pagination/repricing/export; its refresh button selects the latest archive. Usage-over-time rows show USD costs beneath the uncached input, cached input and output counts. These use the existing `cost.components_usd`: uncached cost combines `input + cache_write`, cached cost uses `cached_input`, and output cost uses `output` (including reasoning). Unpriced rows show unavailable component costs; displayed amounts are rounded independently, while API/export components retain decimal strings.

Coverage is **recorded**, **partial**, or **unavailable**; recorded does not prove complete session activity. Unknown/invalid records, cumulative resets and conflicting identities can leave gaps. Costs include only supported recorded tokens; partial or unpriced usage yields a **priced subtotal**, not a complete estimate. No-usage and wholly unpriced results are null, not zero. Tool fees, regional uplifts, Fast/Batch/Flex tiers, subscription charges, taxes and activity in separate child sessions are excluded. Forked rollouts can include inherited history; do not sum these session reports as unique workspace spending. Historical rewrites use the selected raw version even if older normalized events retain previous values.

Verification: the [redacted real usage slice](../examples/fixtures/codex-real-usage-slice.md) preserves six responses with nonzero caching/reasoning and a cumulative mirror reset. [Real-slice API tests](../backend/tests/test_real_usage.py) verify exact totals, weighted percentages, pagination and export. Supplementary [usage regressions](../backend/tests/test_usage.py) cover the redacted real excerpt, mirrors/retries, cumulative deltas/resets, model changes, cache and reasoning arithmetic, thresholds, missing/invalid data, archive selection, pagination and JSON export. [UI regressions](../frontend/tests/usage.test.cjs) cover unavailable/subtotal labels, small values and escaped model names. This does not establish coverage of every Codex token format.
...
**Mandatory snapshot fingerprints — schema v8:** `rollout_line_fingerprints` stores `(session_id, sequence, sha256)` for every imported Codex physical line, including blank lines. `sequence` is one-based; `sha256` is the 32-byte digest of UTF-8 line bytes after removing trailing CR/LF bytes. Other whitespace remains significant. LF versus CRLF and a missing final newline compare equal; exact raw archives still preserve those differences when enabled. Distinct hashes at the same session/line position are retained. Any conflicting previously observed hash in the incoming prefix prevents mirror/gap reconciliation, including after a return to an older snapshot. This is conservative conflict detection, not rewritten-history synchronization.

Fingerprint persistence is part of import correctness and remains active when `raw_archive` is off; it retains no source text. Migration to schema v8 hashes existing archives whose mapping version starts with `codex-jsonl-`, without renormalizing events or creating field mappings. Sessions without archived source bytes acquire fingerprints on explicit reimport. Migration runs once under the startup transaction; older binaries cannot open the migrated schema. Schema v9 additionally creates the independent raw-capture tables described in §3.5; it neither fabricates capture history nor renormalizes existing rows.
...
Implemented 2026-09-07: purpose classification reads nonempty normalized `user`/`assistant` event text from one source. Prefer `codex_jsonl`, then `replay`, `otlp_log`, `otlp_trace`, otherwise the lexically first available source. Order OTLP messages by `start_time,row_id` because their sequence numbers restart per batch; other sources use `sequence,row_id`. Equal OTLP timestamps retain ingestion order. Source selection and traversal use one SQLite read transaction. Ignore blank text and the exact `[REDACTED]` / `[Prompt content not recorded]` placeholders (case-insensitive, after trimming). Other redaction formats are not detected. No usable conversation text or an unattributed telemetry identity produces an error without calling the model or replacing a saved label.

Render each selected event as `kind: text` plus a newline, keeping the first `max_model_chars` characters (default 60,000); one final message may be cut. `truncated=true` means additional selected text was omitted. This prefix can miss a later change of purpose. Tool calls/outputs, blocking question answers, raw records, timing, and session metadata are excluded. User-role context and automated requests are still subject to [DQ-01](data-quality-gaps.md). Choosing one source removes cross-source duplication, but does not reconstruct original model context or merge complementary telemetry.

The [prompt template](../backend/agentboard/prompts/session_purpose.txt), rendered with the [shared taxonomy](../backend/agentboard/classification.py), asks for the dominant intended outcome and a short reason. The enum-backed `ClassificationLabel` is the generated JSON Schema and local validation contract: exactly `category` and `reason`, a canonical enum value and a nonblank reason of at most 2,000 characters. Only external `ClassificationRequest` accepts the `debugging` alias and normalizes it to `bug-fixing`. `ClassificationResult` validates AgentBoard-supplied metadata before storage; API responses use the same result type. See the [structured contract](session-purpose.md#structured-output-contract). See [category definitions](session-purpose.md#categories). Internal results store `taxonomy_version`, `input_selection`, `input_source`, `input_event_ids`, `input_chars`, `truncated`, and SHA-256 hashes of the exact UTF-8 transcript and system prompt, plus `output_schema_sha256` over UTF-8 JSON of the output schema with sorted keys and compact separators, and `classified_at` (UTC RFC 3339). `GET /classification-input` exposes that same bounded context without invoking a model. For example, `user: Calculate the mean\n` is 25 characters before hashing; tool text does not contribute to that input.

The optional OpenAI-compatible gateway selects the configured model or first model returned by discovery, and records model/provider/API/dummy identity. `model_api=chat_completions` (default) calls `/chat/completions` with temperature 0 and at most 600 output tokens. `model_api=responses` calls `/responses` with the same role/content messages as `input`, `store=false`, and at most 2,048 output tokens (including reasoning); sampling defaults are left to the provider. It requires a completed response without an error and nonempty SDK `output_text`; incomplete, failed, and empty responses raise errors without saving or falling back. Classification additionally sends the shared strict schema (`text.format` for Responses, `response_format.json_schema` for Chat Completions). Replay omits structured formatting. Refusals and unfinished Chat Completions classifications also raise errors. Generated JSON is validated directly, without removing Markdown fences or accepting extra fields; unsupported structured-output requests are not retried as plain JSON. This sends no tools and does not request server-side conversation persistence. See the [Responses API reference](https://developers.openai.com/api/reference/resources/responses/methods/create). Auto mode falls back to a deterministic dummy on missing client or connection/timeout failure; reachable service rejection is surfaced, not disguised as dummy success. Dummy classification uses ordered keyword rules, not a model. Internal results set `content_origin=model_generated` or `inferred` for dummy.

External classification PUT forbids unknown fields and validates category/reason/model shape and an optional 64-digit lowercase hexadecimal `input_sha256`. It records `provider=external`, `dummy=false`, `content_origin=model_generated`, `provenance=externally_asserted`, taxonomy version, and save time. These are caller-asserted model results; the model run and input hash are not verified. Classification overwrites the previous value; reimports preserve it. There is no result history, automatic invalidation when a session grows, retained full input snapshot, model artifact revision, confidence calibration, or reproducibility guarantee. Existing stored classifications are not migrated or enriched; rerun explicitly to obtain new metadata.
...
**Tool-result boundary correction (2026-09-16):** a tool event's sequence identifies its call, while its stored output may come from after the selected input. Replay checks the verified archived call/result line numbers. A result before the input is retained; a later result becomes `[No result recorded before the selected input]`, preserving the call arguments. Record order governs even when timestamps tie or run backward. Missing verified result links cause a reimport error before any model call. This applies to ordinary tools and blocking input requests. Existing saved replays are unchanged; recreate them from the original rollout to remove previously retained future results. [Synthetic regressions](../backend/tests/test_api_models.py) cover equal timestamps, both sides of the boundary, missing links and repeated branching.
